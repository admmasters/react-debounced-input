...
test('should render an empty text input box', t => {
  ...
  const wrapper = shallow(<TextInputBox value="" onChange={onChange} />)
...
test('should have optional placeholder text', t => {
  const wrapper = shallow(<TextInputBox value="" className="test-class" />);
...
test('should be able to return a debounced string', t => {
  ...
  const wrapper = mount(<TextInputBox value="" onDebounce={onDebounce} onChange={onChange} />);
  wrapper.find('input').simulate('change', { target: { value: 'example' } });
});
